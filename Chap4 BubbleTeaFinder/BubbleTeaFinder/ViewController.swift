...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    do {
      let batchResult = try coreDataStack.managedContext.execute(batchUpdate) as? NSBatchDeleteResult
      print("Records updated \(String(describing: batchResult?.result))")
    } catch let error as NSError {
      print("Could noe update \(error), \(error.userInfo)")
    }
...
/*
 1. Обратите внимание, что  асинхронный fetchRequest не заменяет обычный fetchRequest. Скорее, вы можете думать об асинхронном fetchRequest как об оболочке вокруг уже имеющегося у вас fetchRequest.

 2. Чтобы создать NSAsynchronousFetchRequest, вам понадобятся две вещи: простой старый NSFetchRequest и complition handler. Выбранные вами места проведения содержатся в свойстве finalResult NSAsynchronousFetchResult. В complition handler вы обновляете свойство venues и перезагружаете табличное представление.

 3. Недостаточно указать complition handler! Вам все равно нужно выполнить запрос асинхронной fetchRequest. Опять же, свойство managedContext CoreDataStack берет на себя всю тяжелую работу за вас. Однако обратите внимание, что вы используете другой метод - на этот раз это execute (_ :) вместо обычного fetch (_ :).
 */
